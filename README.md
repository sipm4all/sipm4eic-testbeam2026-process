...
Run `process/scripts/checker.sh` first when you want a non-destructive sanity pass over decoded per-FIFO files before launching calibration or full processing. The checker writes run-level `*.good-fifos.list` and `*.bad-fifos.list` diagnostic files that show which FIFOs are consistent with the run-level spill structure.
