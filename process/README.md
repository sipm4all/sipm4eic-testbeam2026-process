...
The optional `checker` program scans decoded per-FIFO files before processing and writes ASCII `.check` reports with word counts and spill-counter consistency checks. It is read-only with respect to ROOT data products, but it also writes run-level `*.good-fifos.list` and `*.bad-fifos.list` files. Pass the good list explicitly to `dcalib.sh` or `process.sh` with `--good-fifo-list` to skip FIFOs that would make the checker fail.
