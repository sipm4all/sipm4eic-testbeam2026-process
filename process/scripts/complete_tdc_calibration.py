...
def write_output(filename, rows, expected, fallback):
    ...
    with open(filename, 'w') as f:
        f.write("# TDC calibration file with global fallback rows\n")
        f.write("# generated by complete_tdc_calibration.py\n")
        f.write("# measured concrete rows are written first; wildcard fallback rows are written last\n")
        f.write("[TDC]\n")
        ...
        f.write("\n[TDC]\n")
        f.write("# global fallback rows for missing concrete TDC calibrations\n")
        ...
        for tdc in sorted(fallback):
            row = fallback[tdc]
            f.write("# global_tdc_median tdc={tdc} n={n} off_rms={off_rms:.8g} iif_rms={iif_rms:.8g}\n".format(
                tdc=tdc, n=row.nsource, off_rms=row.off_rms, iif_rms=row.iif_rms))
            f.write(f"* * * * {tdc} {row.off:.10g} {row.iif:.10g}\n")
...
def write_report(filename, input_file, output_file, rows, expected, fallback, single_global):
    ...
    with open(filename, 'w') as f:
        ...
        f.write(f"- global fallback rows written: {len(fallback)}\n")
        f.write(f"- rows outside expected geometry: {len(extra)}\n\n")
        f.write("Missing concrete rows are not filled explicitly. Instead, the output file appends low-specificity wildcard rows in a separate `[TDC]` section. Existing concrete rows remain more specific and therefore take precedence in `calibrator`.\n\n")

        f.write("## Global Fallback Rows\n\n")
        f.write("| tdc | off | iif | source rows | off rms | iif rms |\n")
        f.write("|---:|---:|---:|---:|---:|---:|\n")
        for tdc in sorted(fallback):
            r = fallback[tdc]
...
def main():
    ap = argparse.ArgumentParser(description="Add global [TDC] fallback rows for missing concrete TDC calibrations.")
    ...
    print(f"global fallback rows: {len(fallback)}")
